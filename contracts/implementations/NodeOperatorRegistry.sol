...
contract NodeOperatorRegistry is INodeOperatorRegistry, Initializable {
        ...
        )
    {
        ...
        for (
            ...
            for (
                ...
                uint128 operatorActiveKeys = getOperatorForOperatorId[
                    operatorId
                ...
                if (
                    operatorActiveKeys == 0 &&
                    operatorAssignedDetails[operatorId] == 0
                ) {
            ...
            if (foundOperatorId != 0) {
                ...
                validatorDetails[
                    foundValidators
                ] = operatorIdToValidatorDetails[foundOperatorId].at(
                    smallestOperatorActiveKeys
                );
    ...
    function usePubKeysForValidatorSetup(
        ...
        for (uint256 i; i < _pubKeys.length; i++) {
            ...
            if (
                keccak256(deletedValidatorDetails.pubKey) !=
                keccak256(_pubKeys[i])
            ) {
            ...
            bytes32 encoded = _encodeOperatorIdAndKeyIndex(
                operatorId,
                nextKeyIndex
            );
    ...
    function deletePendingValidators(
        ...
        for (uint128 i; i < _pubKeys.length; i++) {
            ...
            uint128 operatorActiveValidators = getOperatorForOperatorId[
                operatorId
            ].activeValidators;
    ...
    function getPoRAddressList(
        ...
        for (uint256 i; i < numAddresses; i++) {
            uint256 values = uint256(
                activeValidatorIndexes.at(i + _startIndex)
            );
    ...
    {
        activeValidators = getOperatorForOperatorId[_operatorId]
            .activeValidators;
