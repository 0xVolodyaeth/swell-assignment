    ...
    ERC20Upgradeable
{
    ...
    function setSwellTreasuryRewardPercentage(
        ...
        if (
            nodeOperatorRewardPercentage + _newSwellTreasuryRewardPercentage >
            1 ether
        ) {
    ...
    function setNodeOperatorRewardPercentage(
        ...
        // Joined percentage total cannot exeed 100% (1 ether)
        if (
            swellTreasuryRewardPercentage + _newNodeOperatorRewardPercentage >
            1 ether
        ) {
            revert RewardPercentageTotalOverflow();
        }
    ...
    function reprice(
        ...
        if (lastRepriceUNIX != 0) {
            ...
            if (repriceDiff > maximumRepriceDiff) {
                revert RepriceDifferenceTooLarge(
                    repriceDiff,
                    maximumRepriceDiff
                );
            }
        ...
        if (swETHSupplyDiff > maximumswETHDiff) {
            revert RepriceswETHDifferenceTooLarge(
                swETHSupplyDiff,
                maximumswETHDiff
            );
        }
            ...
            if (nodeOperatorRewards != 0) {
                ...
                for (uint128 i = 1; i <= totalOperators; ) {
                    ...
                    if (operatorActiveValidators != 0) {
                        uint256 operatorsRewardShare = wrap(
                            operatorActiveValidators
                        )
